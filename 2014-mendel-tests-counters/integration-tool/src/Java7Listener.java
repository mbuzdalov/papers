// Generated by ANTLR 4.0
import org.antlr.v4.runtime.tree.*;
import org.antlr.v4.runtime.Token;

public interface Java7Listener extends ParseTreeListener {
	void enterInnerCreator(Java7Parser.InnerCreatorContext ctx);
	void exitInnerCreator(Java7Parser.InnerCreatorContext ctx);

	void enterContinueState(Java7Parser.ContinueStateContext ctx);
	void exitContinueState(Java7Parser.ContinueStateContext ctx);

	void enterExpressionList(Java7Parser.ExpressionListContext ctx);
	void exitExpressionList(Java7Parser.ExpressionListContext ctx);

	void enterThrowState(Java7Parser.ThrowStateContext ctx);
	void exitThrowState(Java7Parser.ThrowStateContext ctx);

	void enterTernaryExpr(Java7Parser.TernaryExprContext ctx);
	void exitTernaryExpr(Java7Parser.TernaryExprContext ctx);

	void enterTypeDeclaration(Java7Parser.TypeDeclarationContext ctx);
	void exitTypeDeclaration(Java7Parser.TypeDeclarationContext ctx);

	void enterForUpdate(Java7Parser.ForUpdateContext ctx);
	void exitForUpdate(Java7Parser.ForUpdateContext ctx);

	void enterAnnotation(Java7Parser.AnnotationContext ctx);
	void exitAnnotation(Java7Parser.AnnotationContext ctx);

	void enterEnumConstant(Java7Parser.EnumConstantContext ctx);
	void exitEnumConstant(Java7Parser.EnumConstantContext ctx);

	void enterIfState(Java7Parser.IfStateContext ctx);
	void exitIfState(Java7Parser.IfStateContext ctx);

	void enterNormalClassDeclaration(Java7Parser.NormalClassDeclarationContext ctx);
	void exitNormalClassDeclaration(Java7Parser.NormalClassDeclarationContext ctx);

	void enterImportDeclaration(Java7Parser.ImportDeclarationContext ctx);
	void exitImportDeclaration(Java7Parser.ImportDeclarationContext ctx);

	void enterEnumConstantName(Java7Parser.EnumConstantNameContext ctx);
	void exitEnumConstantName(Java7Parser.EnumConstantNameContext ctx);

	void enterQualifiedIdentifierList(Java7Parser.QualifiedIdentifierListContext ctx);
	void exitQualifiedIdentifierList(Java7Parser.QualifiedIdentifierListContext ctx);

	void enterWhileState(Java7Parser.WhileStateContext ctx);
	void exitWhileState(Java7Parser.WhileStateContext ctx);

	void enterCallExpr(Java7Parser.CallExprContext ctx);
	void exitCallExpr(Java7Parser.CallExprContext ctx);

	void enterVariableDeclarators(Java7Parser.VariableDeclaratorsContext ctx);
	void exitVariableDeclarators(Java7Parser.VariableDeclaratorsContext ctx);

	void enterElementValuePairs(Java7Parser.ElementValuePairsContext ctx);
	void exitElementValuePairs(Java7Parser.ElementValuePairsContext ctx);

	void enterSwitchBlock(Java7Parser.SwitchBlockContext ctx);
	void exitSwitchBlock(Java7Parser.SwitchBlockContext ctx);

	void enterOrExpr(Java7Parser.OrExprContext ctx);
	void exitOrExpr(Java7Parser.OrExprContext ctx);

	void enterAnnotationMethod(Java7Parser.AnnotationMethodContext ctx);
	void exitAnnotationMethod(Java7Parser.AnnotationMethodContext ctx);

	void enterEnumConstants(Java7Parser.EnumConstantsContext ctx);
	void exitEnumConstants(Java7Parser.EnumConstantsContext ctx);

	void enterCatchClause(Java7Parser.CatchClauseContext ctx);
	void exitCatchClause(Java7Parser.CatchClauseContext ctx);

	void enterConstantExpression(Java7Parser.ConstantExpressionContext ctx);
	void exitConstantExpression(Java7Parser.ConstantExpressionContext ctx);

	void enterEnumDeclaration(Java7Parser.EnumDeclarationContext ctx);
	void exitEnumDeclaration(Java7Parser.EnumDeclarationContext ctx);

	void enterSuperMethodExpr(Java7Parser.SuperMethodExprContext ctx);
	void exitSuperMethodExpr(Java7Parser.SuperMethodExprContext ctx);

	void enterTypeParameter(Java7Parser.TypeParameterContext ctx);
	void exitTypeParameter(Java7Parser.TypeParameterContext ctx);

	void enterExplicitConstructorInvocation(Java7Parser.ExplicitConstructorInvocationContext ctx);
	void exitExplicitConstructorInvocation(Java7Parser.ExplicitConstructorInvocationContext ctx);

	void enterInterfaceMethodDeclaratorRest(Java7Parser.InterfaceMethodDeclaratorRestContext ctx);
	void exitInterfaceMethodDeclaratorRest(Java7Parser.InterfaceMethodDeclaratorRestContext ctx);

	void enterCastExpr(Java7Parser.CastExprContext ctx);
	void exitCastExpr(Java7Parser.CastExprContext ctx);

	void enterStatementExpression(Java7Parser.StatementExpressionContext ctx);
	void exitStatementExpression(Java7Parser.StatementExpressionContext ctx);

	void enterVariableInitializer(Java7Parser.VariableInitializerContext ctx);
	void exitVariableInitializer(Java7Parser.VariableInitializerContext ctx);

	void enterBlock(Java7Parser.BlockContext ctx);
	void exitBlock(Java7Parser.BlockContext ctx);

	void enterConstantDeclaratorsRest(Java7Parser.ConstantDeclaratorsRestContext ctx);
	void exitConstantDeclaratorsRest(Java7Parser.ConstantDeclaratorsRestContext ctx);

	void enterDoWhileState(Java7Parser.DoWhileStateContext ctx);
	void exitDoWhileState(Java7Parser.DoWhileStateContext ctx);

	void enterInterfaceMethodOrFieldDecl(Java7Parser.InterfaceMethodOrFieldDeclContext ctx);
	void exitInterfaceMethodOrFieldDecl(Java7Parser.InterfaceMethodOrFieldDeclContext ctx);

	void enterTryState(Java7Parser.TryStateContext ctx);
	void exitTryState(Java7Parser.TryStateContext ctx);

	void enterFieldDeclaration(Java7Parser.FieldDeclarationContext ctx);
	void exitFieldDeclaration(Java7Parser.FieldDeclarationContext ctx);

	void enterNormalInterfaceDeclaration(Java7Parser.NormalInterfaceDeclarationContext ctx);
	void exitNormalInterfaceDeclaration(Java7Parser.NormalInterfaceDeclarationContext ctx);

	void enterSumExpr(Java7Parser.SumExprContext ctx);
	void exitSumExpr(Java7Parser.SumExprContext ctx);

	void enterExplicitGenericInvocation(Java7Parser.ExplicitGenericInvocationContext ctx);
	void exitExplicitGenericInvocation(Java7Parser.ExplicitGenericInvocationContext ctx);

	void enterNotExpr(Java7Parser.NotExprContext ctx);
	void exitNotExpr(Java7Parser.NotExprContext ctx);

	void enterParExpression(Java7Parser.ParExpressionContext ctx);
	void exitParExpression(Java7Parser.ParExpressionContext ctx);

	void enterSwitchLabel(Java7Parser.SwitchLabelContext ctx);
	void exitSwitchLabel(Java7Parser.SwitchLabelContext ctx);

	void enterTypeParameters(Java7Parser.TypeParametersContext ctx);
	void exitTypeParameters(Java7Parser.TypeParametersContext ctx);

	void enterAnnotationTypeElement(Java7Parser.AnnotationTypeElementContext ctx);
	void exitAnnotationTypeElement(Java7Parser.AnnotationTypeElementContext ctx);

	void enterClassDeclaration(Java7Parser.ClassDeclarationContext ctx);
	void exitClassDeclaration(Java7Parser.ClassDeclarationContext ctx);

	void enterShiftExpr(Java7Parser.ShiftExprContext ctx);
	void exitShiftExpr(Java7Parser.ShiftExprContext ctx);

	void enterBooleanLiteral(Java7Parser.BooleanLiteralContext ctx);
	void exitBooleanLiteral(Java7Parser.BooleanLiteralContext ctx);

	void enterIdentifierState(Java7Parser.IdentifierStateContext ctx);
	void exitIdentifierState(Java7Parser.IdentifierStateContext ctx);

	void enterTypeName(Java7Parser.TypeNameContext ctx);
	void exitTypeName(Java7Parser.TypeNameContext ctx);

	void enterArguments(Java7Parser.ArgumentsContext ctx);
	void exitArguments(Java7Parser.ArgumentsContext ctx);

	void enterFormalParameters(Java7Parser.FormalParametersContext ctx);
	void exitFormalParameters(Java7Parser.FormalParametersContext ctx);

	void enterTypeArgument(Java7Parser.TypeArgumentContext ctx);
	void exitTypeArgument(Java7Parser.TypeArgumentContext ctx);

	void enterThisExpr(Java7Parser.ThisExprContext ctx);
	void exitThisExpr(Java7Parser.ThisExprContext ctx);

	void enterForInit(Java7Parser.ForInitContext ctx);
	void exitForInit(Java7Parser.ForInitContext ctx);

	void enterReturnState(Java7Parser.ReturnStateContext ctx);
	void exitReturnState(Java7Parser.ReturnStateContext ctx);

	void enterIndexExpr(Java7Parser.IndexExprContext ctx);
	void exitIndexExpr(Java7Parser.IndexExprContext ctx);

	void enterMethodCallExpr(Java7Parser.MethodCallExprContext ctx);
	void exitMethodCallExpr(Java7Parser.MethodCallExprContext ctx);

	void enterFieldExpr(Java7Parser.FieldExprContext ctx);
	void exitFieldExpr(Java7Parser.FieldExprContext ctx);

	void enterBound(Java7Parser.BoundContext ctx);
	void exitBound(Java7Parser.BoundContext ctx);

	void enterVariableDeclarator(Java7Parser.VariableDeclaratorContext ctx);
	void exitVariableDeclarator(Java7Parser.VariableDeclaratorContext ctx);

	void enterAnnotationTypeDeclaration(Java7Parser.AnnotationTypeDeclarationContext ctx);
	void exitAnnotationTypeDeclaration(Java7Parser.AnnotationTypeDeclarationContext ctx);

	void enterResources(Java7Parser.ResourcesContext ctx);
	void exitResources(Java7Parser.ResourcesContext ctx);

	void enterBitOrExpr(Java7Parser.BitOrExprContext ctx);
	void exitBitOrExpr(Java7Parser.BitOrExprContext ctx);

	void enterQualifiedIdentifier(Java7Parser.QualifiedIdentifierContext ctx);
	void exitQualifiedIdentifier(Java7Parser.QualifiedIdentifierContext ctx);

	void enterMulExpr(Java7Parser.MulExprContext ctx);
	void exitMulExpr(Java7Parser.MulExprContext ctx);

	void enterAssignExpr(Java7Parser.AssignExprContext ctx);
	void exitAssignExpr(Java7Parser.AssignExprContext ctx);

	void enterAssertState(Java7Parser.AssertStateContext ctx);
	void exitAssertState(Java7Parser.AssertStateContext ctx);

	void enterFormalParameterVariables(Java7Parser.FormalParameterVariablesContext ctx);
	void exitFormalParameterVariables(Java7Parser.FormalParameterVariablesContext ctx);

	void enterCompExpr(Java7Parser.CompExprContext ctx);
	void exitCompExpr(Java7Parser.CompExprContext ctx);

	void enterMemberDecl(Java7Parser.MemberDeclContext ctx);
	void exitMemberDecl(Java7Parser.MemberDeclContext ctx);

	void enterElementValueArrayInitializer(Java7Parser.ElementValueArrayInitializerContext ctx);
	void exitElementValueArrayInitializer(Java7Parser.ElementValueArrayInitializerContext ctx);

	void enterNewCallExpr(Java7Parser.NewCallExprContext ctx);
	void exitNewCallExpr(Java7Parser.NewCallExprContext ctx);

	void enterAnnotationName(Java7Parser.AnnotationNameContext ctx);
	void exitAnnotationName(Java7Parser.AnnotationNameContext ctx);

	void enterEnhancedForControl(Java7Parser.EnhancedForControlContext ctx);
	void exitEnhancedForControl(Java7Parser.EnhancedForControlContext ctx);

	void enterPrimary(Java7Parser.PrimaryContext ctx);
	void exitPrimary(Java7Parser.PrimaryContext ctx);

	void enterBlockState(Java7Parser.BlockStateContext ctx);
	void exitBlockState(Java7Parser.BlockStateContext ctx);

	void enterClassBody(Java7Parser.ClassBodyContext ctx);
	void exitClassBody(Java7Parser.ClassBodyContext ctx);

	void enterPrimaryExpr(Java7Parser.PrimaryExprContext ctx);
	void exitPrimaryExpr(Java7Parser.PrimaryExprContext ctx);

	void enterDefaultValue(Java7Parser.DefaultValueContext ctx);
	void exitDefaultValue(Java7Parser.DefaultValueContext ctx);

	void enterVariableModifier(Java7Parser.VariableModifierContext ctx);
	void exitVariableModifier(Java7Parser.VariableModifierContext ctx);

	void enterCreatedName(Java7Parser.CreatedNameContext ctx);
	void exitCreatedName(Java7Parser.CreatedNameContext ctx);

	void enterInterfaceDeclaration(Java7Parser.InterfaceDeclarationContext ctx);
	void exitInterfaceDeclaration(Java7Parser.InterfaceDeclarationContext ctx);

	void enterPackageDeclaration(Java7Parser.PackageDeclarationContext ctx);
	void exitPackageDeclaration(Java7Parser.PackageDeclarationContext ctx);

	void enterTypeRef(Java7Parser.TypeRefContext ctx);
	void exitTypeRef(Java7Parser.TypeRefContext ctx);

	void enterSwitchState(Java7Parser.SwitchStateContext ctx);
	void exitSwitchState(Java7Parser.SwitchStateContext ctx);

	void enterBitAndExpr(Java7Parser.BitAndExprContext ctx);
	void exitBitAndExpr(Java7Parser.BitAndExprContext ctx);

	void enterConstantDeclaratorRest(Java7Parser.ConstantDeclaratorRestContext ctx);
	void exitConstantDeclaratorRest(Java7Parser.ConstantDeclaratorRestContext ctx);

	void enterConstantDeclarator(Java7Parser.ConstantDeclaratorContext ctx);
	void exitConstantDeclarator(Java7Parser.ConstantDeclaratorContext ctx);

	void enterTypeArguments(Java7Parser.TypeArgumentsContext ctx);
	void exitTypeArguments(Java7Parser.TypeArgumentsContext ctx);

	void enterClassCreatorRest(Java7Parser.ClassCreatorRestContext ctx);
	void exitClassCreatorRest(Java7Parser.ClassCreatorRestContext ctx);

	void enterForState(Java7Parser.ForStateContext ctx);
	void exitForState(Java7Parser.ForStateContext ctx);

	void enterModifier(Java7Parser.ModifierContext ctx);
	void exitModifier(Java7Parser.ModifierContext ctx);

	void enterInterfaceBody(Java7Parser.InterfaceBodyContext ctx);
	void exitInterfaceBody(Java7Parser.InterfaceBodyContext ctx);

	void enterPackageOrTypeName(Java7Parser.PackageOrTypeNameContext ctx);
	void exitPackageOrTypeName(Java7Parser.PackageOrTypeNameContext ctx);

	void enterClassBodyDeclaration(Java7Parser.ClassBodyDeclarationContext ctx);
	void exitClassBodyDeclaration(Java7Parser.ClassBodyDeclarationContext ctx);

	void enterForControl(Java7Parser.ForControlContext ctx);
	void exitForControl(Java7Parser.ForControlContext ctx);

	void enterTypeList(Java7Parser.TypeListContext ctx);
	void exitTypeList(Java7Parser.TypeListContext ctx);

	void enterLocalVariableDeclaration(Java7Parser.LocalVariableDeclarationContext ctx);
	void exitLocalVariableDeclaration(Java7Parser.LocalVariableDeclarationContext ctx);

	void enterBitXorExpr(Java7Parser.BitXorExprContext ctx);
	void exitBitXorExpr(Java7Parser.BitXorExprContext ctx);

	void enterAndExpr(Java7Parser.AndExprContext ctx);
	void exitAndExpr(Java7Parser.AndExprContext ctx);

	void enterVariableDeclaratorId(Java7Parser.VariableDeclaratorIdContext ctx);
	void exitVariableDeclaratorId(Java7Parser.VariableDeclaratorIdContext ctx);

	void enterBreakState(Java7Parser.BreakStateContext ctx);
	void exitBreakState(Java7Parser.BreakStateContext ctx);

	void enterCompilationUnit(Java7Parser.CompilationUnitContext ctx);
	void exitCompilationUnit(Java7Parser.CompilationUnitContext ctx);

	void enterElementValue(Java7Parser.ElementValueContext ctx);
	void exitElementValue(Java7Parser.ElementValueContext ctx);

	void enterSemicolonState(Java7Parser.SemicolonStateContext ctx);
	void exitSemicolonState(Java7Parser.SemicolonStateContext ctx);

	void enterNewCreatorExpr(Java7Parser.NewCreatorExprContext ctx);
	void exitNewCreatorExpr(Java7Parser.NewCreatorExprContext ctx);

	void enterClassOrInterfaceType(Java7Parser.ClassOrInterfaceTypeContext ctx);
	void exitClassOrInterfaceType(Java7Parser.ClassOrInterfaceTypeContext ctx);

	void enterFormalParameterDeclarations(Java7Parser.FormalParameterDeclarationsContext ctx);
	void exitFormalParameterDeclarations(Java7Parser.FormalParameterDeclarationsContext ctx);

	void enterBlockStatement(Java7Parser.BlockStatementContext ctx);
	void exitBlockStatement(Java7Parser.BlockStatementContext ctx);

	void enterInterfaceMemberDecl(Java7Parser.InterfaceMemberDeclContext ctx);
	void exitInterfaceMemberDecl(Java7Parser.InterfaceMemberDeclContext ctx);

	void enterIntegerLiteral(Java7Parser.IntegerLiteralContext ctx);
	void exitIntegerLiteral(Java7Parser.IntegerLiteralContext ctx);

	void enterCreator(Java7Parser.CreatorContext ctx);
	void exitCreator(Java7Parser.CreatorContext ctx);

	void enterInterfaceGenericMethodDecl(Java7Parser.InterfaceGenericMethodDeclContext ctx);
	void exitInterfaceGenericMethodDecl(Java7Parser.InterfaceGenericMethodDeclContext ctx);

	void enterStateExpState(Java7Parser.StateExpStateContext ctx);
	void exitStateExpState(Java7Parser.StateExpStateContext ctx);

	void enterTryStatement(Java7Parser.TryStatementContext ctx);
	void exitTryStatement(Java7Parser.TryStatementContext ctx);

	void enterSynchronizedState(Java7Parser.SynchronizedStateContext ctx);
	void exitSynchronizedState(Java7Parser.SynchronizedStateContext ctx);

	void enterEqExpr(Java7Parser.EqExprContext ctx);
	void exitEqExpr(Java7Parser.EqExprContext ctx);

	void enterMethodDeclaration(Java7Parser.MethodDeclarationContext ctx);
	void exitMethodDeclaration(Java7Parser.MethodDeclarationContext ctx);

	void enterConstructorDeclaration(Java7Parser.ConstructorDeclarationContext ctx);
	void exitConstructorDeclaration(Java7Parser.ConstructorDeclarationContext ctx);

	void enterResource(Java7Parser.ResourceContext ctx);
	void exitResource(Java7Parser.ResourceContext ctx);

	void enterVoidInterfaceMethodDeclaratorRest(Java7Parser.VoidInterfaceMethodDeclaratorRestContext ctx);
	void exitVoidInterfaceMethodDeclaratorRest(Java7Parser.VoidInterfaceMethodDeclaratorRestContext ctx);

	void enterElementValuePair(Java7Parser.ElementValuePairContext ctx);
	void exitElementValuePair(Java7Parser.ElementValuePairContext ctx);

	void enterInstanceofExpr(Java7Parser.InstanceofExprContext ctx);
	void exitInstanceofExpr(Java7Parser.InstanceofExprContext ctx);

	void enterIfStatement(Java7Parser.IfStatementContext ctx);
	void exitIfStatement(Java7Parser.IfStatementContext ctx);

	void enterInterfaceMethodOrFieldRest(Java7Parser.InterfaceMethodOrFieldRestContext ctx);
	void exitInterfaceMethodOrFieldRest(Java7Parser.InterfaceMethodOrFieldRestContext ctx);

	void enterSuperExpr(Java7Parser.SuperExprContext ctx);
	void exitSuperExpr(Java7Parser.SuperExprContext ctx);

	void enterMethodBody(Java7Parser.MethodBodyContext ctx);
	void exitMethodBody(Java7Parser.MethodBodyContext ctx);

	void enterArrayInitializer(Java7Parser.ArrayInitializerContext ctx);
	void exitArrayInitializer(Java7Parser.ArrayInitializerContext ctx);

	void enterUnaryExpr(Java7Parser.UnaryExprContext ctx);
	void exitUnaryExpr(Java7Parser.UnaryExprContext ctx);

	void enterPostExpr(Java7Parser.PostExprContext ctx);
	void exitPostExpr(Java7Parser.PostExprContext ctx);

	void enterPrimitiveType(Java7Parser.PrimitiveTypeContext ctx);
	void exitPrimitiveType(Java7Parser.PrimitiveTypeContext ctx);

	void enterNonWildcardTypeArguments(Java7Parser.NonWildcardTypeArgumentsContext ctx);
	void exitNonWildcardTypeArguments(Java7Parser.NonWildcardTypeArgumentsContext ctx);

	void enterClassOrInterfaceDeclaration(Java7Parser.ClassOrInterfaceDeclarationContext ctx);
	void exitClassOrInterfaceDeclaration(Java7Parser.ClassOrInterfaceDeclarationContext ctx);

	void enterArrayCreatorRest(Java7Parser.ArrayCreatorRestContext ctx);
	void exitArrayCreatorRest(Java7Parser.ArrayCreatorRestContext ctx);

	void enterLiteral(Java7Parser.LiteralContext ctx);
	void exitLiteral(Java7Parser.LiteralContext ctx);

	void enterSwitchBlockStatementGroup(Java7Parser.SwitchBlockStatementGroupContext ctx);
	void exitSwitchBlockStatementGroup(Java7Parser.SwitchBlockStatementGroupContext ctx);
}